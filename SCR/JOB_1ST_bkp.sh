...
AS_OF_DATE=`grep -i -A 1 'as-of-date' i-mizuho-commodity-index | awk 'NR==2 {print $2}'  | tr -d '\r'`
...
#echo "**********debug**********"
  ...
  then
    ...
    then
      ...
      cat ${UPDATE_wti} | awk -F',' -v UPDATE_DATE=${LAST_DATE_M} '{ gsub("-", "", $1) ; if( $1 >= UPDATE_DATE && NR >= 2 ) print substr($1, 1, 4)"-"substr($1, 5, 2)"-"substr($1, 7, 2)","$2 }' >> ${MASTER}
    fi
